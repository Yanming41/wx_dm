...
#### 主动推送(不回复任何消息，直接给主人发通知)

外部程序想主动发一条通知(不是回复某条`inbox.jsonl`里的消息)，追加一行到`outbox.jsonl`，不带`reply_to`，改带`sender`：

```json
{"sender": "小红书爬虫", "text": "发现一条新的地毯listing：xxx"}
```

| 字段 | 说明 |
|---|---|
| `sender` | 起一个名字，标识这条消息是哪个程序/agent发的，比如`"小红书爬虫"` |
| `text` | 要发送的文本内容 |

本脚本会把这条消息发给`owner_user_id`——也就是**第一个给这个bot发过消息的微信账号**(本脚本自己从收到的第一条`inbox.jsonl`记录里记下来的，不需要手动配置任何东西)，实际发到微信上的文本会自动加上标签，变成：
...
如果`owner_user_id`还没确定(比如全新部署、还没人给bot发过消息)，这条推送会失败并在控制台打印错误——先用微信给bot发一句话，本脚本才知道该把消息发给谁。
...
- **想发送回复时**：直接给这个 WebSocket 连接发：
  ...
  {"reply_to": "550e8400-...", "text": "这是要发出去的回复内容"}
  ```
  或者主动推送(不带`reply_to`，改带`sender`，格式跟文件接口的主动推送一样)：
  ```json
  {"sender": "小红书爬虫", "text": "发现一条新的地毯listing：xxx"}
  ```
...
```json
{
  ...
  "owner_user_id": "o9cq800kum_xxx@im.wechat"
}
```
